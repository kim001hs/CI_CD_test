...
class UserControllerTest {
    ...
    @Test
    @DisplayName("POST /register - 회원가입 성공")
    ...
    @Test
    @DisplayName("POST /register - 중복 아이디 회원가입 실패")
    ...
    @Test
    @DisplayName("POST /login - 존재하지 않는 아이디로 로그인 실패")
    ...
    @Test
    @DisplayName("POST /login - 잘못된 비밀번호로 로그인 실패")
    ...
    @Test
    @DisplayName("GET /users - 모든 사용자 조회")
    ...
    @Test
    @DisplayName("GET /users/{id} - ID로 사용자 조회 성공")
    void getUserById() throws Exception {
        ...
        mockMvc.perform(get("/users/{id}", registered.getId()))
                .andExpect(status().isOk());
...
//                .andExpect(jsonPath("$.name").value("테스트유저"));
    ...
    @Test
    @DisplayName("GET /users/{id} - 존재하지 않는 ID로 조회 실패")
    ...
    void fullUserFlowScenario() throws Exception {
        ...
        // 3. 사용자 조회
        mockMvc.perform(get("/users/{id}", registeredUser.getId()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.userId").value("testuser"))
                .andExpect(jsonPath("$.name").value("테스트유저"));
